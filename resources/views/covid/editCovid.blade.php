...
<link rel="stylesheet" href="../../css/covid/editCovid.css">

@section('content')

    <h1>Editando dados de COVID de <span class="azul">{{$covid->paciente->nome_paciente}}</span></h1>
    <form action="/covid/update/{{$covid->id_covid}}" class="formulario" method="POST">
        @csrf
        @method('put')
        <div class="boxCampos">
            ...
            <div class="form-row">
                <div class="form-group col-md-3">
                    ...
                    <select class="form-control" name="primeira_dose" id="dose" required>
                        <option value="{{$covid->primeira_dose}}">{{$covid->primeira_dose}} </option>
                ...
                <div class="form-group col-md-3">
                    ...
                    <select class="form-control" name="segunda_dose" id="dose" required>
                        <option value="{{$covid->sedunda_dose}}">{{$covid->segunda_dose}}</option>
                        @foreach ($vacina as $v1 )
                            <option value="{{$v1->nome_vacina}}">{{$v1->nome_vacina}}</option>
                        @endforeach
                        <option value="nao vacinou">Ainda não se vacinou</option>
                ...
                <div class="form-group col-md-3">
                    ...
                    <select class="form-control" name="terceira_dose" id="dose" required>
                        <option value="{{$covid->terceira_dose}}">{{$covid->terceira_dose}}</option>
                        @foreach ($vacina as $v1 )
                            <option value="{{$v1->nome_vacina}}">{{$v1->nome_vacina}}</option>
                        @endforeach
                        <option value="nao vacinou">Ainda não se vacinou</option>
            ...
            <div class="resultadoEData">
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="data_testagem" class="form-control col-md-3" value="{{$covid->data_testagem}}">
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" name="celular" id="contato" class="form-control" value="{{$covid->celular}}" required>
                </div>
                <div class="form-group" id="telefoneResidencial">
                    <label>Residencial</label>
                    <input type="text" name="residencial"  id="residencial" class="form-control" value="{{$covid->residencial == "" ? "" : $covid->residencial}}">
                </div>
            </div>
        ...
        <script src="../../js/covid/editCovid.js"></script>
@endsection
